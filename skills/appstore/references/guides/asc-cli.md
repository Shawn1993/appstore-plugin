...
## macOS 公证（App Store 外分发才需要）

```bash
asc notarization submit --file "App.zip" --wait --poll-interval 30s --timeout 1h
asc notarization log <SUBMISSION_ID>        # Invalid 时看具体原因
```

坑：Developer ID 证书只能网页创建（API 不支持）；公证 Invalid 三大常因——嵌套二进制未签名、缺 hardened runtime、内嵌库缺时间戳；签名链必须 Developer ID App → Developer ID CA → Apple Root，自定义 trust 设置会破坏链。
