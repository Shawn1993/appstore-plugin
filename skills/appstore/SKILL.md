...
- `assets/` — 模板：metadata-template（提审资料全字段）、audit-report-template.md/.html（审计报告的结构与网页样式）、delivery-report-template.html（全流程一体化交付页）、UploadOptions.plist（直传配置）
...
- **全流程（上架一条龙）最终收尾于一体化交付页**：写到项目 `.appstore/delivery-report.html`，填充 `assets/delivery-report-template.html`——五个 tab 装下全部产出：总览（判定灯+流程进度）/ 商店物料（ASC 每个输入框一张字段卡，标字符数、一键复制）/ 截图（按档位分区的画廊，img 指向项目内实际文件）/ 审计（沿用审计三规则）/ 待办。对话里只报总判定和文件路径。只做单项任务（只审计/只文案）时仍用对应单项交付，不强行套壳
