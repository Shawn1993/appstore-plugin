...
- **审计报告默认收尾于可视化 HTML 文件**：写到项目 `.appstore/audit-report.html`，对话里只报总判定和文件路径；**不主动发布 claude.ai Artifact**，用户明确要分享链接才发布。网页填充 `assets/audit-report-template.html`（判定灯/分区卡/依据 chip/术语 tooltip 样式全内置，禁止重写 CSS），结构与写作三规则见 audit-report-template.md（依据超链接溯源、标题说人话讲后果、术语带 hover 解释）；md 归档版同规则
