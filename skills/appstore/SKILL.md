...
**文件地图**（按知识来源分三组）：
- `references/guides/` — 自有方法论：asc-cli（CLI 手册）、aso-playbook（ASO 运营）、screenshots（截图生产）、regions（地区合规）、derived-app-audit（模板/壳衍生 App 专项）
...
- `assets/` — 模板：metadata-template（提审资料全字段）、audit-report-template.md/.html（审计报告的结构与网页样式）、UploadOptions.plist（直传配置）
...
- **问必答题用 AskUserQuestion 卡片**，不要散文追问：能给选项的都给选项（付费模式 = 免费/付费/含内购；目标市场 = 多选），挖不到的才留 Other 让用户填。一次卡片问完，不分多轮
- **开场亮作战地图**：走全流程时先给六阶段 checklist（采集→物料→构建→ASC→审计→提交），每完成一段勾一段再继续——用户随时知道走到哪
- **长产出交文件不刷屏**：提审资料等成体系产出写成文件交付（复制粘贴友好），对话里只给摘要
- **审计报告默认收尾于可视化网页**：产出双份——归档 md 文件 + Artifact 网页（判定灯置顶、红/黄/绿分区卡片、每条带证据与修复动作），对话里只报总判定和链接；环境不支持 Artifact 时退回纯文件。网页版直接填充 `assets/audit-report-template.html`（样式/判定灯/依据 chip/术语 tooltip 全部内置，禁止重写 CSS），并遵守 audit-report-template.md 的写作三规则：依据全部超链接可溯源、标题说人话讲后果、术语带 hover 解释
- **报告用固定模板**：审计报告按 `assets/audit-report-template.md`（总判定灯 + 三段表格 + 下一步），提审资料按 `assets/metadata-template.md`——不许自由发挥结构

## 0. 智能采集（用户友好的核心：能自己挖的绝不问）

用户只需给**一个入口**：产品网站 URL、H5 地址、或项目 repo 路径。按下表自动挖掘，全部挖完后**一次性**产出「采集确认卡」——已解析结果表 + AskUserQuestion 必答题卡，不要一路追问：
...
**必答题控制在 ≤5 个**（挖不到才问）：ASC API Key 位置、付费模式、目标市场（决定走 references/guides/regions.md 哪些合规门槛，含中国区则追问 App 备案服务号）、审核联系人电话邮箱。
