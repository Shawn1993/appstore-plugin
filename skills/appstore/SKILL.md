...
- `assets/` — 模板：metadata-template（提审资料全字段）、UploadOptions.plist（直传配置）
...
- **审计报告默认收尾于可视化网页**：产出双份——归档 md 文件 + Artifact 网页（判定灯置顶、红/黄/绿分区卡片、每条带证据与修复动作），对话里只报总判定和链接；环境不支持 Artifact 时退回纯文件
