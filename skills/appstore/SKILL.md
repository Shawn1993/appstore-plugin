...
- **审计报告默认收尾于可视化 HTML 文件**：写到项目 `.appstore/audit-report.html`（判定灯置顶、红/黄/绿分区卡片、依据可点溯源、术语 hover 解释），对话里只报总判定和文件路径，用户浏览器打开即看；**不主动发布到 claude.ai Artifact**，仅当用户明确要可分享链接时才发布。网页直接填充 `assets/audit-report-template.html`（样式全内置，禁止重写 CSS），并遵守 audit-report-template.md 的写作三规则
